...
def physics_loss(v_target, v_predicted, HyperParams):
    ...
    ```
    """
    norm_target = torch.norm(v_target, dim=1)
    norm_predicted = torch.norm(v_predicted, dim=1)

    term1 = torch.norm(v_target - v_predicted, dim=1)**2 / (2 - norm_target**2)
    term2 = HyperParams.delta * torch.norm((v_target / (HyperParams.epsilon + \
                norm_target.unsqueeze(-1))) - (v_predicted / (HyperParams.epsilon + norm_predicted.unsqueeze(-1))), dim=1)**2

    total_loss = torch.sum(term1 + term2)
